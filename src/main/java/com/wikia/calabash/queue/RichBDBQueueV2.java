 ...
 */
public class RichBDBQueueV2<T> {

    private static final Logger LOG = LoggerFactory.getLogger(RichBDBQueueV2.class);
     ...
     */
    public RichBDBQueueV2(final File queueBaseDir, final String queueName, final int cacheSize, final Function<TypeFactory, JavaType>  getJavaType) throws IOException, DatabaseException {
        _mapper = createObjectMapper();
        _q = new BDBQueue(queueBaseDir.getAbsolutePath(), queueName, cacheSize);
        _javaType = getJavaType.apply(_mapper.getTypeFactory());
    }
     ...
     */
    public void push(final T object) throws IOException, DatabaseException {
     ...
     */
    public void consume(final Consumer<T> consumer) throws DatabaseException, InterruptedException {
        while (_doRun) {
     ...
     */
    private boolean processMessage(final Consumer<T> consumer) throws DatabaseException {
    ...
    public T peek() throws DatabaseException {
        final byte[] data = _q.peek();
    ...
    private T deserialize(@Nullable final byte[] data) throws DatabaseException {
        if (data == null) {
            return null;
        }
        try {
            return _mapper.readValue(data, 0, data.length, _javaType);
